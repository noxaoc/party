...
import { faAngleDown, faAngleUp, faArrowDown, faArrowUp, faEdit, faExternalLinkAlt, faTrashAlt } from '@fortawesome/free-solid-svg-icons';
...
//import Tab from '@themesberg/react-bootstrap/Tab';
...
import { Participant, ParticipantEvent } from "../data/participants";
...
export const ParticipantsTable = ( props ) => {
  ...
  const TableRow = (props) => {
    const { pid, num, surname, patronymic, name, club, regDate, role, phone, email, price, paid } = props;
    let dt_arr = R.split(' ', dateTimeToStr(regDate) )
...
*/}
    ...
    */}
              <Dropdown.Item onClick={(_)=>setShowDlg( {showDlg:true,currPid:pid} )}>
                <FontAwesomeIcon icon={faEdit} className="me-2" /> Редактировать
              </Dropdown.Item>

              <Dropdown.Item className="text-danger" onClick={doRemoveParticipant(pid)}>
                <FontAwesomeIcon icon={faTrashAlt} className="me-2"  /> Удалить
              </Dropdown.Item>
            </Dropdown.Menu>
          </Dropdown>
    ...
    <Card border="light" className="table-wrapper table-responsive shadow-sm">
      ...
      <Card.Body className="pt-0 pb-1 px-2">
        <Table hover className="user-table align-items-center">
          ...
          <tbody>
            {participants.map(t => <TableRow key={`participant-${t.pid}`} {...t} />)}
          </tbody>
        </Table>
        <Card.Footer className="px-1 py-2 border-0 d-flex justify-content-start">
          <small className="fw-bold">
